...
exports.MARKET_DICTIONARY = void 0;
exports.matchMarketName = matchMarketName;
exports.parseFloorAndRoom = parseFloorAndRoom;
exports.parseSmartOrderText = parseSmartOrderText;
// 동대문 상가 표준 명칭 및 별칭(줄임말/은어) 매핑 사전
exports.MARKET_DICTIONARY = [
 ...
 */
function matchMarketName(token, exactOnly) {
    ...
    var withoutParen = trimmed.replace(/\([^)]*\)/g, '').trim().toUpperCase();
    if (withoutParen && !candidates.includes(withoutParen)) {
        candidates.push(withoutParen);
    }
    // 1. 정확 일치 (Exact match) 우선 - 별칭 길이 내림차순
    for (var _i = 0, MARKET_DICTIONARY_1 = exports.MARKET_DICTIONARY; _i < MARKET_DICTIONARY_1.length; _i++) {
        var entry = MARKET_DICTIONARY_1[_i];
        var sortedAliases = __spreadArray([], entry.aliases, true).sort(function (a, b) { return b.length - a.length; });
        for (var _a = 0, sortedAliases_1 = sortedAliases; _a < sortedAliases_1.length; _a++) {
            var alias = sortedAliases_1[_a];
    ...
    for (var _c = 0, MARKET_DICTIONARY_2 = exports.MARKET_DICTIONARY; _c < MARKET_DICTIONARY_2.length; _c++) {
        var entry = MARKET_DICTIONARY_2[_c];
        var sortedAliases = __spreadArray([], entry.aliases, true).sort(function (a, b) { return b.length - a.length; });
        for (var _d = 0, sortedAliases_2 = sortedAliases; _d < sortedAliases_2.length; _d++) {
            var alias = sortedAliases_2[_d];
            var uAlias = alias.toUpperCase();
            if (alias.length < 2) {
                // 영문 1글자(예: D)인 경우에만 부분 일치 허용 (맨 앞이거나 기호 뒤)
                if (/^[A-Za-z]$/.test(alias)) {
                    for (var _e = 0, candidates_2 = candidates; _e < candidates_2.length; _e++) {
                        var cand = candidates_2[_e];
                        var regex = new RegExp("(^|[^A-Za-z])".concat(alias, "(?![A-Za-z])"), 'i');
                        if (regex.test(cand)) {
                            return { canonical: entry.canonical, matchedAlias: alias };
                        }
                    }
            ...
            for (var _f = 0, candidates_3 = candidates; _f < candidates_3.length; _f++) {
                var cand = candidates_3[_f];
 ...
 */
function parseFloorAndRoom(text) {
    // 1. 지하 / B층 패턴 (예: B1-12, B1 12호, 지하1층 5호, 지1 5, B2-45)
    var bPattern = /(?:지하\s*(\d+)|지\s*(\d+)|B\s*(\d+)|b\s*(\d+))(?:(?:\s*층|\s*F|\s*f)[\s\-\/\.호~]*|[\s\-\/\.호~]*)([가-힣A-Za-z0-9\s\-~]+?)(?:\s*호|$|\s+(?=[가-힣A-Za-z]))/i;
    var bMatch = text.match(bPattern);
    if (bMatch && (bMatch[1] || bMatch[2] || bMatch[3] || bMatch[4])) {
        var floorNum = bMatch[1] || bMatch[2] || bMatch[3] || bMatch[4];
        var floor = "\uC9C0\uD558".concat(floorNum, "\uCE35");
        var room = bMatch[5] ? bMatch[5].trim() : '';
        if (room && !room.endsWith('호') && /^\d+$/.test(room)) {
            room = "".concat(room, "\uD638");
        }
        return { floor: floor, room: room, matchedText: bMatch[0] };
    }
    // 2. 일반 지상층 + 호수 패턴 (예: 3-12, 3/12, 3.12, 3층 12호, 3F 12호, 4층 가동 15호, 3층 C열 12호)
    var standardPattern = /(\d+)(?:(?:\s*층|\s*F|\s*f)[\s\-\/\.호~]*|[\-\/\.])([가-힣A-Za-z0-9\s\-~]+?)(?:\s*호|$|\s+(?=[가-힣A-Za-z]))/;
    var stdMatch = text.match(standardPattern);
    if (stdMatch) {
        var floor = "".concat(stdMatch[1], "\uCE35");
        var room = stdMatch[2].trim();
        if (room && !room.endsWith('호') && /^\d+$/.test(room)) {
            room = "".concat(room, "\uD638");
        }
        return { floor: floor, room: room, matchedText: stdMatch[0] };
    }
    // 3. 단순 층만 있는 경우 (예: 3층, 4F, B1층)
    var onlyFloorPattern = /(\d+)\s*(?:층|F|f)/i;
    var flMatch = text.match(onlyFloorPattern);
    if (flMatch) {
        return { floor: "".concat(flMatch[1], "\uCE35"), room: '', matchedText: flMatch[0] };
    }
    // 4. 동만 있는 경우 (예: C동, 가동, 에이동)
    var dongPattern = /([가-힣A-Za-z]+)\s*동(?:\s|$)/;
    var dongMatch = text.match(dongPattern);
    if (dongMatch) {
        return { floor: "".concat(dongMatch[1], "\uB3D9"), room: '', matchedText: dongMatch[0].trim() };
    }
    return null;
}
/**
 ...
 */
function parseSmartOrderText(inputText, defaultStoreName) {
    if (defaultStoreName === void 0) { defaultStoreName = ''; }
    ...
    var currentGroupStore = footerStoreName || defaultStoreName.trim();
    ...
    for (var lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        if (rawLine.includes('상호/건물명/층/호수') ||
            rawLine.startsWith('===') ||
            rawLine.startsWith('---') ||
        ...
        var arrivalMatch = rawLine.match(/^([가-힣A-Za-z0-9_\s]{2,25})(?:으로|로)부터\s*사입\s*요청서/);
        if (arrivalMatch) {
            var cand = arrivalMatch[1].trim();
        ...
        var storeHeaderMatch = rawLine.match(/^(?:[\[★■▶◆\*●📍📌🏷️🏢🏪\s]*)(?:소매명|소매상호|소매점|소매처|소매|상호명|상호|고객사|매장명)\s*[:：]?\s*([가-힣A-Za-z0-9_\s]{2,25})(?:[\]★■▶◆\*]|\s*$)/);
        if (storeHeaderMatch) {
            var candidate = storeHeaderMatch[1].trim();
        ...
        var cleanLine = rawLine.replace(/^(?:[\[★■▶◆\*●📍📌🏷️🏢🏪\s]*)(?:주문내용|주문상세|주문목록|주문서|주문)\s*[:：~-]?\s*/i, '').trim();
        // 2. 줄 번호 제거 (예: "1. ", "1) ", "① ")
        ...
        if (cleanLine.includes('/') || (cleanLine.includes(',') && cleanLine.split(',').length >= 3)) {
            var delimiter = cleanLine.includes('/') ? '/' : ',';
            var parts = cleanLine.split(delimiter).map(function (p) { return p.trim(); });
            if (parts.length >= 2) {
                var store = '';
                var market = '';
                var floor = '';
                ...
                var firstAsMarket = matchMarketName(parts[0]);
                if (firstAsMarket) {
                    market = firstAsMarket.canonical;
                    // parts[1]이 층/호수를 모두 포함하는지 확인 (예: "2층 18")
                    var fr = parseFloorAndRoom(parts[1] || '');
                    if (fr && fr.room) {
                        floor = fr.floor;
                        room = fr.room;
                    ...
                    var secondAsMarket = matchMarketName(parts[1] || '');
                    market = secondAsMarket ? secondAsMarket.canonical : (parts[1] || '');
                    var fr = parseFloorAndRoom(parts[2] || '');
                    if (fr && fr.room) {
                        floor = fr.floor;
                        room = fr.room;
                        remark = parts.slice(3).join(' / ');
                ...
                if (floor && !room) {
                    var fr2 = parseFloorAndRoom(floor);
                    if (fr2 && fr2.room) {
                        floor = fr2.floor;
                        room = fr2.room;
                    }
                ...
                results.push({
                    id: "parsed_".concat(Date.now(), "_").concat(lineIndex, "_").concat(Math.random().toString(36).substring(2, 6)),
                    rawText: rawLine,
                    store: store.trim(),
        ...
        var remainingTokens = __spreadArray([], tokens, true);
        // 복합 건물명 탐색 (예: "APM" "플레이스" 2개 토큰) 우선 확인
        var marketFoundIndex = -1;
        for (var i = 0; i < remainingTokens.length - 1; i++) {
            var combo = "".concat(remainingTokens[i], " ").concat(remainingTokens[i + 1]);
            var match = matchMarketName(combo, true); // 복합 건물명은 완전 일치만 허용
            if (match) {
        ...
        if (!detectedMarket) {
            for (var i = 0; i < remainingTokens.length; i++) {
                var match = matchMarketName(remainingTokens[i]);
                if (match) {
                    detectedMarket = match.canonical;
                    marketFoundIndex = i;
                    // 만약 띄어쓰기 없이 붙어있는 경우(예: 디오트4층G16), 건물명만 지우고 나머지는 남김
                    var token = remainingTokens[i];
                    var aliasRegex = new RegExp(match.matchedAlias.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
                    var replaced = token.replace(aliasRegex, ' ').trim();
                    if (replaced) {
                        var newPieces = replaced.split(/\s+/);
                        remainingTokens.splice.apply(remainingTokens, __spreadArray([i, 1], newPieces, false));
                    }
                    else {
        ...
        var restText = remainingTokens.join(' ');
        var frResult = parseFloorAndRoom(restText);
        if (frResult) {
            detectedFloor = frResult.floor;
            detectedRoom = frResult.room;
            // 층/호수에 매칭된 단어들을 남은 텍스트에서 제거
            var cleanedRest = restText.replace(frResult.matchedText, ' ').trim();
        ...
        if ((detectedMarket === '디오트' || detectedMarket === '디자이너클럽' || detectedMarket === '디자이너') && detectedRoom) {
            var noFullNames = !/디오트|디자이너/i.test(rawLine);
            var hasD = /디|d/i.test(rawLine.replace(/\s+/g, ''));
            if (noFullNames && hasD) {
        ...
        // 결과 판정
        var hasMarket = !!detectedMarket;
        var hasFloorOrRoom = !!detectedFloor || !!detectedRoom;
        var confidence = (hasMarket && hasFloorOrRoom) ? 'high' : hasMarket ? 'medium' : 'low';

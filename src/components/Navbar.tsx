...
import { Database, Calculator, Store, LogOut, Users, HandCoins, MessageSquare, Layers, Settings } from 'lucide-react';
    ...
    <header className="bg-indigo-900 text-white shadow-md z-30">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2.5 flex justify-between gap-2 items-start">
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          ...
          {/* Top Right: Actions & User Info & Logout */}
          <div className="flex items-center justify-end gap-1.5 sm:gap-2 flex-wrap">
            ...
            <div 
              ...
              className="bg-indigo-950/80 hover:bg-indigo-950 border border-indigo-700/60 px-2 sm:px-2.5 py-1 rounded-xl text-indigo-100 font-bold text-[11px] sm:text-xs flex items-center gap-1.5 cursor-pointer transition shadow-xs shrink-0"
              title="내 정보 열기"
            >
            ...
            <button
              type="button"
              onClick={onOpenProfile}
              className="hidden sm:inline-flex bg-indigo-800/80 hover:bg-indigo-700 px-2 py-1 rounded-xl text-[11px] font-bold transition text-indigo-100 shrink-0"
            >
              내 정보
            </button>
            {/* Logout Button */}
            <button
              type="button"
              onClick={onLogout}
              className="px-2.5 sm:px-3 py-1 sm:py-1 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-[11px] sm:text-xs font-bold transition flex items-center gap-1 shadow-xs shrink-0 h-[26px] sm:h-[28px]"
              title="로그아웃"
            >
          ...
          <div className="flex items-center justify-end gap-1.5 flex-wrap">
            {(isBuyer || isAdmin) && onOpenBuyerWorkday && (
              <button
                ...
                className="bg-blue-600 hover:bg-blue-500 text-white px-2 sm:px-2.5 py-1 rounded-xl text-[11px] sm:text-xs font-bold transition flex items-center gap-1 shadow-xs shrink-0"
              >
            ...
            {isLocal && (
              <button
                ...
                className="bg-emerald-700 hover:bg-emerald-600 px-2 sm:px-2.5 py-1 rounded-xl text-[11px] sm:text-xs font-bold transition flex items-center gap-1 shrink-0"
              >

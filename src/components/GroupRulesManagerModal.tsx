...
export const GroupRulesManagerModal: React.FC<GroupRulesManagerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[300] flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto p-5 border border-gray-800 my-auto">
        <div className="flex items-center justify-between mb-4 border-b pb-3">
          ...
          <button type="button" onClick={onClose} className="text-gray-500 hover:text-gray-200 p-1">
            <X className="w-5 h-5" />
          </button>
        </div>

        {currentUser?.role === 'admin' && (
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3 mb-4 bg-gray-900 p-3 rounded-xl border border-gray-800 text-xs">
            <div>
              <label className="font-bold text-gray-200 block mb-1">주문 상호 *</label>
              <input
                type="text"
                required
                value={storeName}
                ...
                placeholder="예: 호치강남"
                className="w-full border border-gray-700 rounded-lg p-2 bg-gray-900 outline-none focus:ring-2 focus:ring-violet-500"
              />
            </div>
            <div>
              <label className="font-bold text-gray-200 block mb-1">대표 거래처 *</label>
              <input
                type="text"
                required
                value={groupName}
            ...
            <div>
              <label className="font-bold text-gray-200 block mb-1">적용 시작일 *</label>
              <input
                type="date"
                required
                value={effectiveFrom}
            ...
            <div>
              <label className="font-bold text-gray-200 block mb-1">일치 규칙</label>
              <select
                value={matchType}
        ...
        <div className="border border-gray-800 rounded-xl overflow-hidden text-xs">
          <div className="px-3.5 py-2 bg-gray-950 font-bold text-gray-200 border-b border-gray-800">
            현재 등록된 규칙 ({rules.length}개)
          </div>
          <div className="divide-y divide-gray-800 max-h-52 overflow-y-auto">
            {rules.length === 0 ? (
              <div className="p-4 text-center text-gray-500">등록된 규칙이 없습니다.</div>
            ) : (
              rules.map(r => (
                <div key={r.id} className="p-3 flex items-center justify-between gap-2 hover:bg-gray-900">

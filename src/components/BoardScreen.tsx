...
export const BoardScreen: React.FC<BoardScreenProps> = ({ currentUser, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 w-screen h-screen bg-slate-50 z-[180] overflow-y-auto pt-16">
      {/* Top Navigation Bar */}
      <div className="fixed top-0 left-0 w-full h-14 bg-white border-b border-gray-200 z-50 flex items-center px-4 shadow-sm">
        <button onClick={onClose} className="p-2 -ml-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition flex items-center gap-1 font-semibold">
          <ArrowLeft className="w-5 h-5" />
          <span>뒤로</span>
        </button>
        <div className="flex-1 font-bold text-center text-lg text-gray-800 pr-10">소통 게시판</div>
      </div>
      ...
      <div className="space-y-4">
        {posts.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-gray-100">
            ...
            <p className="text-gray-400 text-sm mt-1">첫 번째 글을 남겨보세요!</p>
          </div>
        ) : (
          posts.map(post => (
              ...
              {editingPostId === post.id ? (
                <div className="mb-2">
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-xl px-3 py-2 mb-2 outline-none focus:ring-2 focus:ring-indigo-500 font-bold"
                    placeholder="제목"
                  />
                  <textarea
                    value={editContent}
                    onChange={(e) => setEditContent(e.target.value)}
                    className="w-full border border-gray-300 rounded-xl px-3 py-2 mb-3 outline-none focus:ring-2 focus:ring-indigo-500 min-h-[120px] resize-y"
                    placeholder="내용"
                  />
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <div className="space-y-3 mb-3">
                  {post.comments && Object.entries(post.comments).map(([commentId, comment]: [string, any]) => (
                    <div key={commentId} className="flex justify-between items-start group bg-gray-50 p-2.5 rounded-lg">
                      <div>
                      ...
                      {(currentUser.username === comment.authorUsername || currentUser.role === 'admin') && (
                        <button
                          ...
                          className="p-1 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded transition opacity-0 group-hover:opacity-100 sm:opacity-100"
                          title="댓글 삭제"
                        >
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 border border-gray-300 rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition"
                  />
                  <button
                    onClick={() => handleCommentSubmit(post.id)}
                    disabled={!commentInputs[post.id]?.trim() || submittingComment === post.id}
                    className="px-4 py-2 bg-indigo-50 text-indigo-600 font-semibold rounded-xl text-sm hover:bg-indigo-100 transition disabled:opacity-50"
                  >
                    등록
                  </button>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition font-medium"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition min-h-[160px] resize-y"

...
import { ChevronLeft, ChevronRight, ChevronDown, CalendarCheck, Plus, Edit, Trash2, CheckCircle2, Sparkles, Layers, X } from 'lucide-react';
...
export const CalendarView: React.FC<CalendarViewProps> = React.memo(({
  ...
  const availableYears = useMemo(() => {
    ...
    const minY = Math.min(2022, currentY - 2, year, ...txYears);
    const maxY = Math.max(2030, currentY + 3, year, ...txYears);
    const list: number[] = [];
  ...
  const handleYearChange = (newYear: number) => {
    if (onSelectYearMonth) {
      onSelectYearMonth(newYear, month);
    } else {
      onChangeMonth((newYear - year) * 12);
    }
  };

  const handleMonthChange = (newMonth: number) => {
    if (onSelectYearMonth) {
      onSelectYearMonth(year, newMonth);
    } else {
      ...
      >
        ...
        <div className="flex flex-col gap-2.5 mb-4">
          ...
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-xl font-bold text-slate-900 tracking-tight flex items-center gap-0.5">
              <div className="relative inline-flex items-center">
                <select
                  id="calendarYearSelect"
                  value={year}
                  onChange={(e) => handleYearChange(Number(e.target.value))}
                  className="text-lg sm:text-xl font-bold text-slate-900 bg-transparent hover:bg-slate-100 rounded-lg py-0.5 pl-1.5 pr-5 border border-transparent hover:border-slate-300 cursor-pointer outline-none transition appearance-none tracking-tight"
                  title="연도 선택"
                >
              ...
              <div className="relative inline-flex items-center">
                ...
                >
                  {Array.from({ length: 12 }, (_, i) => (
                    <option key={i} value={i}>{String(i + 1).padStart(2, '0')}월</option>
                  ))}
                </select>
                <ChevronDown className="w-3.5 h-3.5 text-slate-500 absolute right-1 pointer-events-none" />
              </div>
